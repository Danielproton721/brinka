...
<h1>🔧 Diagnóstico de e-mail — BRINKA</h1>
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await resend.emails.send({
      from: fromAddress || "BRINKA Brinquedos <onboarding@resend.dev>",
      to: [to],
      subject: "Teste de e-mail — BRINKA",
    ...
    if (result.error) {
      return NextResponse.json({
        ...
        erro: result.error,
        dica: "Se falar 'domain not verified' ou 'testing emails', você precisa VERIFICAR o seu domínio no Resend (Domains → Add → SPF/DKIM no DNS).",
        diag,
        kv,
      });
    }
