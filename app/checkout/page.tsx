...
// Escassez: aparece SÓ AQUI, no resumo do checkout. A página de produto e a
// gaveta do carrinho não falam de estoque — quem está escolhendo ainda não
// precisa de pressa, e repetir o aviso em toda tela queima o recurso.
//
// A chave é o id da VARIANTE, não o slug: Pista e Canhão dividem o mesmo slug,
// e com ele as duas mostrariam o mesmo "últimas N" lado a lado no resumo — o
// número idêntico entrega que é fabricado. Por id, cada versão tem o seu.
//
// Determinístico de propósito: o mesmo item mostra sempre o mesmo número, em
// qualquer visita e em qualquer aparelho. Um valor que muda a cada carregamento
// é o que faz o cliente perceber que o estoque não é real.
function stockLeftFor(key: string | number) {
  let h = 0
...
const CONFIRMED_ORDER_STORAGE_KEY = 'brinka-confirmed-order-v1';
...
//  1. Chumbado, toda loja clonada deste template nasce disparando a conversao na
//     conta da loja de origem — a venda de uma aparece na campanha da outra e o
//     CPA das duas fica errado.
//  2. O par AW-id/label e publico (vai no gtag, qualquer um le no DevTools).
//     Igual em duas lojas, ele liga uma na outra em dois cliques.
// Sem a env definida, NAO dispara: melhor perder a conversao do que credita-la
// na conta errada.
const GOOGLE_ADS_CONVERSION_SEND_TO = process.env.NEXT_PUBLIC_GOOGLE_ADS_SEND_TO?.trim() || '';
const GOOGLE_ADS_CONVERSION_STORAGE_KEY = 'brinka-google-ads-conversions-v1';
...
function CheckoutContent() {
  ...
  const getInputClass = (field: string) => {
    const base = "w-full px-4 py-3 border rounded-xl bg-[#f4f1ea] focus:bg-white focus:outline-none transition-all ";
    return base + (errors[field] ? "border-red-500 focus:ring-2 focus:ring-red-500/30" : "border-[#e6e0d5] focus:ring-2 focus:ring-[#ff5a1f]/30 focus:border-[#ff5a1f]");
  };
  ...
  if (!isMounted) {
    return <div className="loja-co min-h-screen bg-[#f4f1ea] flex items-center justify-center p-4 text-[#8b8378] font-bold">Carregando checkout...</div>;
  }

  if (items.length === 0 && !pixData && !paymentConfirmed) {
    return (
      <div className="loja-co min-h-screen bg-[#f4f1ea] flex flex-col items-center justify-center p-4">
        <ShoppingBag className="w-16 h-16 text-[#d8d0c2] mb-4" />
        <h2 className="text-xl font-bold text-[#171310] mb-2">Seu carrinho está vazio</h2>
        <p className="text-[#8b8378] mb-6">Adicione produtos para finalizar sua compra.</p>
        <a href="/" className="orange-flow text-white font-bold px-8 py-3 rounded-full uppercase tracking-wider">Voltar para a loja</a>
      </div>
    );
  }
  ...
  if (paymentConfirmed) {
    ...
    return (
      <div className="loja-co min-h-screen bg-gradient-to-b from-emerald-50 via-white to-[#f4f1ea] flex items-center justify-center p-4">
        <motion.div
        ...
        >
          ...
          <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#ff5a1f] via-[#ff9a52] to-[#ff5a1f]" />
          ...
          <button
            ...
            aria-label="Fechar"
            className="absolute top-3 right-3 z-10 flex h-9 w-9 items-center justify-center rounded-full bg-[#efeae1] text-[#8b8378] transition hover:bg-[#e6e0d5] hover:text-[#171310]"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <motion.h1
            ...
            className="text-3xl sm:text-4xl font-black text-[#171310] mb-2 tracking-tight"
          >
            Pedido Confirmado!
          </motion.h1>
          ...
          <motion.p
            ...
            className="text-[#8b8378] font-medium mb-8 text-sm sm:text-base"
          >
            Obrigado pela sua compra, <span className="text-[#171310] font-bold">{(display.customerName || '').split(' ')[0]}</span>! 🎉
          </motion.p>
          ...
          {display.orderCode && (
            <motion.div
              ...
              className="bg-[#171310] rounded-2xl p-5 mb-6 text-left shadow-lg"
            >
              <div className="flex items-start gap-3">
                <PackageCheck className="w-5 h-5 text-[#ff5a1f] shrink-0 mt-0.5" />
              ...
              <button
                ...
                className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-xs font-black uppercase tracking-wide text-[#171310] transition hover:bg-[#fff1ea]"
              >
          ...
          <motion.div
            ...
            className="bg-[#f4f1ea] rounded-2xl p-5 mb-6 text-left space-y-3"
          >
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#8b8378] font-medium">Método</span>
              <span className="font-bold text-[#171310] flex items-center gap-1.5">
            ...
            <div className="border-t border-[#e6e0d5]" />
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#8b8378] font-medium">Total pago</span>
            ...
            {display.paymentMethod === 'card' && display.installments > 1 && (
              <>
                <div className="border-t border-[#e6e0d5]" />
                <div className="flex justify-between items-center text-sm">
                  <span className="text-[#8b8378] font-medium">Parcelas</span>
                  <span className="font-bold text-[#171310]">
            ...
            <div className="border-t border-[#e6e0d5]" />
            <div className="flex justify-between items-center text-sm">
              <span className="text-[#8b8378] font-medium">E-mail</span>
              <span className="font-bold text-[#171310] text-xs">{display.email}</span>
            </div>
          </motion.div>
          ...
          >
            ...
            <p className="text-xs text-emerald-800 font-medium text-left leading-relaxed">
              Enviamos a confirmação para <strong>{display.email}</strong>. O código acima acompanha o pedido na BRINKA; o rastreio da transportadora será enviado quando o pedido for despachado.
            </p>
          </motion.div>
          ...
          <motion.div
            ...
            className="flex items-center justify-center gap-2 text-xs text-[#a89f92] font-bold mb-8"
          >
            <ShieldCheck className="w-4 h-4" />
            Garantia de Satisfação BRINKA
          </motion.div>
          ...
          {display.orderCode && (
            <motion.a
              ...
              className="mb-3 inline-block w-full py-4 bg-[#171310] hover:bg-[#171310] text-white rounded-xl font-black text-sm uppercase tracking-wide shadow-lg transition-all hover:-translate-y-0.5"
            >
              Acompanhar Pedido
            </motion.a>
          )}
          ...
          <motion.button
            ...
            className="inline-block w-full py-4 bg-gradient-to-b from-[#ff6a2f] to-[#f0480d] hover:brightness-105 text-white rounded-xl font-black text-sm uppercase tracking-wide shadow-lg shadow-[#171310]/30 transition-all hover:-translate-y-0.5"
          >
            Continuar Comprando
          </motion.button>
        </motion.div>
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showExitWarning && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-[#171310]/80 backdrop-blur-sm"
          >
            <motion.div
              initial={{ scale: 0.9, y: 20 }}
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="bg-white rounded-3xl p-6 sm:p-8 pt-8 sm:pt-10 max-w-md w-full shadow-2xl relative text-center border-4 border-[#ff5a1f]"
            >
              <button 
                onClick={() => setShowExitWarning(false)}
                className="absolute top-4 right-4 text-[#a89f92] hover:text-[#171310] transition-colors"
                aria-label="Fechar aviso"
              >
                <X size={24} />
              </button>
              
              <h2 className="text-3xl font-black text-[#171310] mb-2 uppercase tracking-tight">Espera aí!</h2>
              <p className="text-[#4b453e] font-medium mb-6 text-sm sm:text-base leading-relaxed">
              ...
              <div className="space-y-3">
                ...
                <button 
                  ...
                  className="w-full bg-[#efeae1] text-[#8b8378] font-bold py-3.5 rounded-xl hover:bg-[#e6e0d5] transition-all uppercase tracking-wider text-xs"
                >
      ...
      <div className="loja-co min-h-screen bg-[#f4f1ea] text-[#171310] pb-12">
      {/* Topbar */}
      <div className="bg-white border-b border-[#efeae1] px-5 py-3 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-2">
          {/* Mesmo selo laranja do <Logo> do site, para o checkout não parecer
              outra marca a meio caminho da compra. */}
          <span className="logo">
            <span className="bolt">
          ...
          <span className="text-[10px] text-[#d9430c] tracking-widest uppercase font-semibold">Checkout</span>
        </div>
      ...
      <div className="max-w-6xl mx-auto mt-6 px-4 grid grid-cols-1 lg:grid-cols-[1.3fr_1fr] gap-6">
        ...
        <div className="space-y-4">
          ...
          <motion.div layout className={`bg-white border rounded-2xl p-6 shadow-sm transition-all duration-300 ${currentStep === 1 ? 'border-[#ff5a1f] ring-1 ring-[#ff5a1f]' : 'border-[#e6e0d5]'}`}>
            <div className="flex items-center justify-between mb-5">
              ...
              {currentStep > 1 && (
                <button onClick={() => setCurrentStep(1)} className="text-[#ff5a1f] text-xs font-bold hover:underline">Editar</button>
              )}
            </div>
            ...
            {currentStep === 1 ? (
              ...
              >
                <div>
                  <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">Nome completo</label>
                  <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Seu nome" className={getInputClass('name')} />
                </div>
                <div>
                  <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">E-mail</label>
                ...
                <button onClick={handleNextToStep2} className="w-full mt-2 py-3.5 bg-gradient-to-b from-[#ff6a2f] to-[#f0480d] text-white font-bold rounded-xl shadow-lg shadow-[#171310]/25 hover:brightness-110 transition-all">
              ...
              <motion.div 
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="text-sm text-[#8b8378] font-medium pt-2"
              >
                Dados preenchidos ✓
              </motion.div>
            )}
          ...
          <motion.div layout className={`bg-white border rounded-2xl p-6 shadow-sm transition-all duration-300 ${currentStep === 2 ? 'border-[#ff5a1f] ring-1 ring-[#ff5a1f]' : 'border-[#e6e0d5] opacity-60'}`}>
            <div className="flex items-center justify-between mb-5">
              ...
              {currentStep > 2 && (
                <button onClick={() => setCurrentStep(2)} className="text-[#ff5a1f] text-xs font-bold hover:underline">Editar</button>
              )}
            </div>
            ...
            {currentStep === 2 && (
              ...
              >
                <div>
                  <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">CEP {isFetchingCep && <span className="text-emerald-500 lowercase normal-case">(buscando...)</span>}</label>
                  <input type="text" value={cep} onChange={handleCepChange} placeholder="00000-000" className={`sm:w-1/3 ${getInputClass('cep')}`} />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-[2fr_1fr] gap-4">
                  <div>
                    <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">Endereço (Rua, Avenida)</label>
                    <input type="text" value={street} onChange={e => setStreet(e.target.value)} placeholder="Nome da rua" className={getInputClass('street')} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">Número</label>
                    <input type="text" value={number} onChange={e => setNumber(e.target.value)} placeholder="Ex: 123" className={getInputClass('number')} />
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">Complemento (opcional)</label>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">Bairro</label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[2fr_1fr] gap-4">
                  <div>
                    <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">Cidade</label>
                    <input type="text" value={city} onChange={e => setCity(e.target.value)} placeholder="Sua cidade" className={getInputClass('city')} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">Estado</label>
                    <input type="text" value={stateUF} onChange={e => setStateUF(e.target.value)} placeholder="UF" className={getInputClass('stateUF')} />
                  </div>
                </div>
                <div className="rounded-2xl border border-[#e6e0d5] bg-[#f4f1ea] p-4">
                  <p className="mb-3 text-xs font-black uppercase tracking-[0.16em] text-[#8b8378]">Opções de frete</p>
                      ...
                      return (
                        <button
                          ...
                          className={`rounded-xl border-2 bg-white p-4 text-left transition-all ${
                            isSelected
                              ? 'border-[#ff5a1f] shadow-[0_10px_25px_rgba(255,90,31,0.16)]'
                              : 'border-[#e6e0d5] hover:border-[#d8d0c2]'
                          }`}
                        >
                          <span className="flex items-center justify-between gap-3">
                            <span className="flex items-center gap-2 text-sm font-black text-[#171310]">
                              <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
                                option.price === 0
                                  ? 'bg-emerald-50 text-emerald-600'
                                  : 'bg-[#ffe4d6] text-[#171310]'
                              }`}>
                            ...
                            <span className={`rounded-full px-2.5 py-1 text-xs font-black ${
                              option.price === 0
                                ...
                                : 'bg-[#ffe4d6] text-[#d9430c]'
                            }`}>
                              {option.price === 0 ? 'Grátis' : `R$ ${option.price.toFixed(2).replace('.', ',')}`}
                            </span>
                          </span>
                          <span className="mt-2 block text-xs font-bold uppercase tracking-wide text-[#8b8378]">{option.eta}</span>
                          <span className="mt-1 block text-xs leading-relaxed text-[#8b8378]">{option.description}</span>
                        </button>
                      );
                    })}
                  </div>
                ...
                {formError && <motion.p initial={{opacity:0}} animate={{opacity:1}} className="text-red-500 text-sm font-bold text-center pt-2">{formError}</motion.p>}
                <button onClick={handleNextToStep3} className="w-full mt-2 py-3.5 bg-gradient-to-b from-[#ff6a2f] to-[#f0480d] text-white font-bold rounded-xl shadow-lg shadow-[#171310]/25 hover:brightness-110 transition-all">
                  Ir para Pagamento
                </button>
              </motion.div>
            )}
          ...
          {/* Sessão 3: Pagamento */}
          <motion.div id="payment-section" layout className={`bg-white border rounded-2xl p-6 shadow-sm transition-all duration-300 ${currentStep === 3 ? 'border-[#ff5a1f] ring-1 ring-[#ff5a1f]' : 'border-[#e6e0d5] opacity-50'}`}>
            <h2 className={`text-lg font-extrabold mb-5 flex items-center gap-2 ${currentStep >= 3 ? 'text-[#171310]' : 'text-[#a89f92]'}`}>
              <span className={`w-6 h-6 rounded-full text-white flex items-center justify-center text-xs ${currentStep >= 3 ? 'bg-[#ff5a1f]' : 'bg-[#d8d0c2]'}`}>3</span>
              Forma de pagamento
            </h2>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  <button 
                    ...
                    className={`p-4 border-2 rounded-xl flex items-center gap-3 font-extrabold transition-all ${payMethod === 'pix' ? 'border-[#ff5a1f] bg-[#ff5a1f]/5 text-[#ff5a1f]' : 'border-[#e6e0d5] bg-white text-[#4b453e] hover:border-[#d8d0c2]'}`}
                  >
                  <img src="https://icons.yampi.me/svg/card-pix.svg" alt="Pix" width="28" height="20" /> PIX
                  </button>
                  <button 
                    onClick={() => setPayMethod('card')}
                    className={`p-4 border-2 rounded-xl flex items-center gap-3 font-extrabold transition-all ${payMethod === 'card' ? 'border-[#ff5a1f] bg-[#ff5a1f]/5 text-[#ff5a1f]' : 'border-[#e6e0d5] bg-white text-[#4b453e] hover:border-[#d8d0c2]'}`}
                  >
                ...
                {payMethod === 'pix' ? (
                  <motion.div 
                    ...
                    exit={{ opacity: 0, x: -10 }}
                    className="bg-gradient-to-b from-[#ff5a1f]/5 to-white border border-dashed border-[#ff5a1f]/40 rounded-xl p-5 text-center flex flex-col items-center"
                  >
                    {!pixData ? (
                      <>
                        <h3 className="font-bold text-[#171310] mb-1">Pague com PIX e libere envio na hora</h3>
                        <p className="text-sm text-[#8b8378] mb-4">Aprovação em segundos. Clique no botão abaixo para gerar seu PIX.</p>
                      ...
                      <>
                        ...
                        <p className="text-xs text-[#8b8378] mb-2">Ou copie o código abaixo (PIX Copia e Cola):</p>
                        <div className="flex items-center gap-2 w-full max-w-sm">
                          <input type="text" readOnly value={pixData.qrCode} className="flex-1 bg-white text-[#4b453e] text-xs p-3 rounded-xl border border-[#e6e0d5] font-mono focus:outline-none" />
                          <button onClick={handleCopyPix} className="bg-[#ff5a1f] text-white text-xs font-bold px-4 py-3 rounded-xl hover:bg-[#171310] transition-colors">COPIAR</button>
                        </div>
                        <div className="mt-4 w-full max-w-sm rounded-2xl border border-[#e6e0d5] bg-white p-3 text-left shadow-sm">
                          <div className="flex items-start gap-3">
                            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#ffe4d6] text-[#171310]">
                              {pixProof ? <FileCheck2 className="h-5 w-5" /> : <Upload className="h-5 w-5" />}
                            </div>
                            <div className="min-w-0 flex-1">
                              <p className="text-xs font-black uppercase tracking-[0.14em] text-[#8b8378]">Comprovante opcional</p>
                              <p className="mt-1 text-xs font-medium leading-relaxed text-[#8b8378]">
                                Se quiser, anexe uma imagem ou PDF do comprovante. A confirmação do PIX continua automática.
                              </p>
                            </div>
                          </div>

                          <label className="mt-3 flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-[#ff5a1f]/60 bg-[#fff1ea] px-4 py-3 text-xs font-black uppercase tracking-wide text-[#171310] transition hover:border-[#ff5a1f] hover:bg-[#ffe4d6]">
                  ...
                  >
                    {cardResult ? (
                      <div className="space-y-3">
                        ...
                        {!cardResult.approved && (
                          <button
                            ...
                            className="w-full py-3 rounded-xl bg-gradient-to-b from-[#ff6a2f] to-[#f0480d] text-white font-bold text-sm hover:brightness-110 transition-all"
                          >
                      ...
                      <>
                        <div>
                          <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">
                            Dados do cartão
                          </label>
                          <div
                            id="pagou-card-element"
                            className="w-full min-h-[120px] px-3 py-3 border border-[#e6e0d5] rounded-xl bg-[#f4f1ea] transition-all"
                          />
                          {!cardSdkReady && !cardSdkError && (
                            <p className="mt-2 text-xs text-[#8b8378] flex items-center gap-1.5">
                              <span className="inline-block h-2 w-2 rounded-full bg-[#ff5a1f] animate-pulse" />
                          ...
                          <p className="mt-2 text-[11px] text-[#a89f92] flex items-center gap-1">
                            <Lock className="w-3 h-3" />
                            Dados do cartão criptografados pela Pagou.ai
                          </p>
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-[#8b8378] uppercase tracking-wide mb-1.5">Parcelas</label>
                          <select
                            value={cardInstallments}
                            onChange={e => setCardInstallments(e.target.value)}
                            className="w-full px-4 py-3 border border-[#e6e0d5] rounded-xl bg-[#f4f1ea] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#ff5a1f]/30 focus:border-[#ff5a1f] transition-all"
                          >
                        ...
                        {cardError && (
                          <div className="space-y-2">
                            ...
                            <button
                              type="button"
                              onClick={retryCardForm}
                              className="w-full py-2.5 rounded-xl border-2 border-[#ff5a1f] text-[#ff5a1f] font-bold text-sm hover:bg-[#ffe4d6] transition-colors"
                            >
                              Tentar novamente
                            </button>
                          </div>
        ...
        <div className="space-y-4">
          <div className="bg-white border border-[#e6e0d5] rounded-2xl p-6 shadow-sm lg:sticky lg:top-24">
            <h2 className="text-lg font-extrabold text-[#171310] mb-5">Resumo do pedido</h2>
            
            <div className="space-y-4 pb-5 border-b border-[#efeae1] mb-5 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
              {items.map((item) => (
                <div key={item.id} className="flex gap-3">
                  <div className="w-16 h-16 rounded-lg overflow-hidden bg-[#efeae1] border border-[#e6e0d5] shrink-0">
                    <img src={item.image} alt={item.name} className="w-full h-full object-contain p-1" />
                  </div>
                  <div className="flex-1 flex flex-col justify-between min-w-0">
                    <strong className="block text-[#171310] font-bold leading-tight text-xs truncate">{item.name}</strong>
                    <span className="mt-0.5 flex items-center gap-1 text-[10px] font-bold text-red-500">
                      <Flame size={10} className="fill-red-500" />
                      Últimas {stockLeftFor(item.id)} unidades — reservado por tempo limitado
                    </span>
                    <div className="flex items-center justify-between mt-1.5">
                      {/* Quantity controls */}
                      <div className="flex items-center border border-[#e6e0d5] rounded-lg overflow-hidden">
                        <button
                          onClick={() => updateQuantity(item.id, item.quantity - 1)}
                          className="w-7 h-7 flex items-center justify-center text-[#8b8378] hover:bg-[#efeae1] transition-colors font-bold text-base"
                        >−</button>
                        <span className="w-7 text-center text-xs font-bold text-[#171310]">{item.quantity}</span>
                        <button
                          onClick={() => updateQuantity(item.id, item.quantity + 1)}
                          className="w-7 h-7 flex items-center justify-center text-[#8b8378] hover:bg-[#efeae1] transition-colors font-bold text-base"
                        >+</button>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-[#171310]">R$ {(item.price * item.quantity).toFixed(2).replace('.', ',')}</span>
                        <button
                          onClick={() => removeItem(item.id)}
                          className="text-[#d8d0c2] hover:text-red-400 transition-colors"
            ...
            <div className="space-y-2.5 mb-5">
              <div className="flex justify-between text-sm text-[#8b8378] font-medium">
                <span>Subtotal</span>
                <span>R$ {totalPrice.toFixed(2).replace('.', ',')}</span>
              </div>
              {couponApplied ? (
                <div className="flex justify-between items-center rounded-lg bg-[#ffe4d6]/50 px-2.5 py-1.5 text-xs font-bold text-[#171310]">
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="rounded-full p-0.5 text-[#171310]/60 transition-colors hover:bg-[#171310]/10 hover:text-[#171310]"
                ...
                <div>
                  <div className="flex gap-2">
                    <input
                      ...
                      className="h-9 min-w-0 flex-1 rounded-lg border border-[#e6e0d5] px-3 text-xs font-semibold uppercase tracking-wide text-[#171310] outline-none placeholder:normal-case placeholder:font-normal placeholder:text-[#a89f92] focus:border-[#ff5a1f]"
                    />
                    <button
                      type="button"
                      onClick={handleApplyCoupon}
                      disabled={!couponInput.trim()}
                      className="h-9 shrink-0 rounded-lg bg-gradient-to-b from-[#ff6a2f] to-[#f0480d] px-4 text-xs font-black uppercase tracking-wide text-white transition-all hover:brightness-105 disabled:opacity-50"
                    >
                      Aplicar
                    </button>
                  </div>
              ...
              <div className="flex justify-between text-sm font-bold text-[#8b8378]">
                <span>Frete</span>
                <span className={shippingPrice === 0 ? 'text-emerald-600' : 'text-[#171310]'}>
                  {shippingPrice === 0 ? 'Grátis' : `R$ ${shippingPrice.toFixed(2).replace('.', ',')}`}
                </span>
              </div>
              <div className="flex justify-between text-xs font-bold uppercase tracking-wide text-[#a89f92]">
                <span>{selectedShipping.name}</span>
                <span>{selectedShipping.eta}</span>
              </div>
              <div className="border-t border-[#efeae1] pt-3 flex justify-between items-baseline text-[#171310] font-black">
                <span>Total</span>
            ...
            <button 
              ...
              className="w-full py-4 bg-[#22c55e] hover:bg-[#16a34a] disabled:bg-[#a89f92] text-white rounded-xl font-black text-sm uppercase tracking-wide shadow-lg transition-all hover:-translate-y-0.5"
            >
              {isGeneratingPix
                ? "GERANDO PIX..."
                : isProcessingCard
            ...
            {/* Bandeiras — mesmo componente do rodapé do site. */}
            <PaymentFlags className="justify-center mt-5 pt-4 border-t border-[#efeae1]" />
          </div>
        </div>
      </div>
    </div>
...
export default function CheckoutPage() {
  return (
    <Suspense fallback={<div className="loja-co min-h-screen bg-[#f4f1ea] flex items-center justify-center font-bold text-[#8b8378]">Iniciando checkout seguro...</div>}>
      <CheckoutContent />
    </Suspense>
  );
}
